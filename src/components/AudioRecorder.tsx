...
const AudioRecorder = ({
  ...
  useEffect(() => {
    ...
    return () => {
      cleanupResources();
      LoggingService.info('audio-recorder', 'Componente AudioRecorder desmontado').catch(err => console.error('Error al registrar desmontaje:', err));
    };
  }, []);
  useEffect(() => {
    if (selectedPatient) {
  ...
  const getSupportedMimeTypes = () => {
    const mimeTypes = ['audio/webm', 'audio/webm;codecs=opus', 'audio/mp4', 'audio/mp4;codecs=opus', 'audio/ogg', 'audio/ogg;codecs=opus', 'audio/wav', 'audio/wav;codecs=1'];
  ...
  const createBackup = () => {
    ...
    try {
      ...
      const backupBlob = new Blob(audioChunksRef.current, {
  ...
  const processBackupRecording = async () => {
    ...
    try {
      ...
      const combinedBlob = new Blob(backupAudios, {
        type: backupAudios[0].type
      });
  ...
  const handlePendingWebhook = () => {
    ...
    saveConsultation(pendingConsultation).then(saveError => {
      if (saveError) {
  ...
  const finalizeConsultation = (transcripcion: string, resumen: string) => {
    ...
    saveConsultation(newConsultation).then(saveError => {
      if (saveError) {
        ...
        <div className="space-y-4">
          <div className="space-b-2">
            ...
            <div className="flex items-center space-x-2">
              <Input id="patientName" placeholder="Ingrese el nombre del paciente" value={patientName} onChange={e => setPatientName(e.target.value)} disabled={isRecording || isProcessing} className="w-full" />
              <Button variant="outline" onClick={() => setShowPatientSelector(!showPatientSelector)} disabled={isRecording || isProcessing}>
                {selectedPatient ? "Cambiar" : "Buscar"}
              </Button>
            </div>
          </div>
          
          {showPatientSelector && <div className="mt-2 border rounded-md p-4 bg-gray-50">
              <PatientSelector onPatientSelect={handlePatientSelect} selectedPatientId={selectedPatient?.id} initialPatientName={patientName} />
            </div>}
          
          {recordingError && <Alert variant="destructive" className="mt-4">
              <AlertTriangle className="h-4 w-4" />
              <AlertTitle>Error en la grabación</AlertTitle>
              <AlertDescription>{recordingError}</AlertDescription>
            </Alert>}
          
          {(isRecording || isProcessing || isUploading) && <div className="mt-4">
              {isRecording ? <div className="space-y-2">
                  ...
                  {backupAudios.length > 0 && <div className="text-xs text-green-600 text-center">
                      {backupAudios.length} respaldos guardados
                    </div>}
                  <div className="flex justify-center">
                    <Button variant="outline" size="sm" onClick={manualBackup} disabled={isBackupSaving} className="text-xs">
                      {isBackupSaving ? <>
                ...
                </div> : isUploading ? <div className="flex items-center justify-center space-x-2">
          ...
          <div className="flex justify-center pt-4">
            {isRecording ? <Button onClick={stopRecording} variant="destructive" size="lg" className="w-full sm:w-auto">
                <Square className="mr-2 h-4 w-4" />
                Detener Grabación
              </Button> : <Button onClick={() => startRecording(false)} variant="default" size="lg" disabled={isProcessing || isUploading || !patientName.trim()} className="w-full sm:w-auto bg-medical-600 hover:bg-medical-700">
